...
qa_lib.check_opt_emptiness() {
    local imagedir="${1}"
    if [ -z "${imagedir}" ]; then
        echo "qa_lib.check_opt_emptiness: missing imagedir parameter" >&2
        return 1
    fi
    if [ ! -d "${imagedir}" ]; then
        echo "qa_lib.check_opt_emptiness: ${imagedir} is not a directory" >&2
        return 1
    fi

    # Make this work with both /opt and /var/opt.
    ...
    local optdir=
    local optdirs=()
    for optdir in "${candidate_opts[@]}"; do
        if [[ -L "${optdir}" && ! -e "${optdir}" ]]; then
            echo "${optdir} is a broken symlink. This is not expected." >&2
            return 1
        fi
        if [ -d "${optdir}" ]; then
            optdirs+=( "${optdir}" )
    ...
    if [[ ${#optdirs[@]} -eq 0 ]]; then
        echo "No /opt nor /var/opt dirs. All good. Nothing to check."
        return 0
    fi

    # /*opt should be absolutely empty (they will break with ostree and packages
    # must respect FHS guidelines anyway. Instead of breaking the seeder build,
    # I prefer to break the releasing process, given that's orders of magniture
    # less expensive.
    # Before 2026-03-01, we just warn, after that we return 1 and fail the release
    # process.
    local optdir=
    for optdir in "${optdirs[@]}"; do
        if ! fs_lib.dir_empty "${optdir}"; then
            echo "WARNING: ${optdir} is not empty. This will break ostree upgrades!" >&2
        fi
    done
}