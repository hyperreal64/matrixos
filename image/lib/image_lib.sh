...
image_lib.setup_vmtest_config() {
    ...
    local console_boot_params="console=ttyS0,115200"
    local systemd_boot_params="systemd.log_color=0"
    local systemd_env_params="systemd.setenv=SYSTEMD_COLORS=0 systemd.setenv=SYSTEMD_URLIFY=0"
    local boot_params="${console_boot_params} ${systemd_boot_params} ${systemd_env_params}"
    cp -v "${ostree_boot_cfg}" "${vmtest_boot_cfg}"
    sed -i "s:splash::g" "${vmtest_boot_cfg}"
    sed -i "s:quiet:${boot_params}:g" "${vmtest_boot_cfg}"
